...
def build_features(
    ...
    if save_as_split_files:
        ...
        print(
            f"Save indices data to {out_file} as {args.train_record_split_size} parts"
        )
        split_and_save_nparray(out_file, data, args.train_record_split_size)
    else:
        np.savez(
            out_file,
            context_idxs=np.array(context_idxs),
...
def split_and_save_nparray(file_name, data, num_parts):
    file_name_template = file_name.removesuffix(".npz") + "_part_{}" + ".npz"
    splited_data = [{} for _ in range(num_parts)]

    for arr in data.keys():
        print(f"Processing {arr} {data[arr].shape}...")
        parts = split_nparray(data[arr], num_parts)
        for i in range(len(parts)):
            splited_data[i][arr] = parts[i]

    for i in range(num_parts):
        np.savez(file_name_template.format(i), **splited_data[i])


def split_nparray(arr, num_parts):
    size = arr.shape[0]
    n_per_part = size // num_parts
    parts = []
    for i in range(num_parts):
        if i == num_parts - 1:
            parts.append(arr[i * n_per_part :])
        else:
            parts.append(arr[i * n_per_part : (i + 1) * n_per_part])
    return parts
